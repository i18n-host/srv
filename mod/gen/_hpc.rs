...
Func::AuthTest => {
  ...
  let headers = &ctx.req.headers;
  match auth::test(args.timezone as i8,args.dpi as u8,args.w as u16,args.h as u16,&args.os_ver,&args.arch,args.cpu_num,&args.gpu,&headers).await {
    Err(err)=>call_err("auth::test", err, captcha, || dvec![args.timezone as i8,args.dpi as u8,args.w as u16,args.h as u16,&args.os_ver,&args.arch,args.cpu_num,&args.gpu,&headers].join(",")).await?,
    Ok(_)=>(State::OK, vec![])
  }
}
